...
Our minimal dataset required to reproduce all study findings is located under TAMF_FRNet\util\evaltool\Prediction. This dataset contains all the data necessary to replicate the results reported in the article. Specifically, the FNR, PR, and F-measure curves can be regenerated by running FNR.py, plot_pr_curves.py, and Fmeasure.py, respectively, using this data.
